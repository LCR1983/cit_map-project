document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        ...
        "news1": "Recommended for weekend trips! Kanto spring gourmet feature", "news2": "Local harvest festivals being held in each prefecture", "news3": "Specialty data with photos and ratings increased to 504 items!",
    ...
    const sendChatMessage = async (text) => {
        ...
        try {
            ...
            // キーワードパターン（食材・季節・県名が含まれる場合）
            const chatPatterns = [
                { re: /ネギ|葱/, res: '群馬の「下仁田ネギ」（冬）や埼玉の「深谷ネギ」（冬）がおすすめです！加熱するとトロトロになる甘さが絶品ですよ。' },
                { re: /海鮮|魚|マグロ|シーフード/, res: '神奈川の「三崎マグロ」（冬）や茨城の「あんこう鍋」（冬）がおすすめです！港直送の鮮度が違います。' },
                { re: /フルーツ|果物|甘い|スイーツ/, res: '栃木の「とちおとめいちご」（春）や茨城の「メロン」（夏・日本一）が最高です！甘さの濃さが段違いですよ。' },
                { re: /野菜|ベジタブル/, res: '群馬の「高原レタス」（夏）や茨城の「れんこん」（秋・生産量日本一）がおすすめです！' },
                { re: /秋|紅葉|収穫/, res: '秋なら茨城の「笠間の栗」、群馬の「まいたけ」、埼玉の「川越芋」が旬です！収穫祭も各地で開催されます。' },
                { re: /夏|暑|涼/, res: '夏なら茨城の「メロン」（日本一）や千葉の「梨」「すいか」が旬！暑さに負けない甘みを楽しめます。' },
                { re: /春|暖|桜/, res: '春なら栃木の「いちご」（日本一）や神奈川の「湘南しらす」が解禁！清々しい春の食材をどうぞ。' },
                { re: /冬|寒|鍋|温/, res: '冬なら茨城の「あんこう鍋」や群馬の「下仁田ネギ」、神奈川の「三崎マグロ」で温まりましょう！' },
                { re: /茨城|いばらき/, res: '茨城はメロン（夏・日本一）、れんこん（秋・日本一）、あんこう鍋（冬）が有名です！「納豆」も忘れずに！' },
                { re: /栃木|とちぎ/, res: '栃木はいちご（春・日本一）、宇都宮餃子、日光湯波が有名です！いちご王国の実力は格別です。' },
                { re: /群馬|ぐんま/, res: '群馬はこんにゃく（全国生産の約90%）、下仁田ネギ（冬）、上州和牛が有名です！' },
                { re: /埼玉|さいたま/, res: '埼玉は狭山茶（春）、川越芋（秋）、深谷ネギ（冬）が有名です！小江戸・川越の芋スイーツも必見。' },
                { re: /千葉|ちば/, res: '千葉は落花生（秋・日本一）、梨（夏・日本一）、イワシが有名です！農業産出額は全国上位の実力県です。' },
                { re: /東京|とうきょう/, res: '東京は江戸前あさり（春）、東京うど（春）、小松菜（冬）、くさや（伊豆諸島）があります！' },
                { re: /神奈川|横浜|湘南|鎌倉/, res: '神奈川は三崎マグロ（冬）、湘南しらす（春）、三浦大根（冬）、かまぼこが有名です！' },
            ];

            // 都県ランダム紹介データ（抽象的な質問用）
            const prefRandomData = [
                {
                    name: '茨城県', emoji: '🍈',
                    highlights: ['メロン（夏・生産量日本一）', 'れんこん（秋・生産量日本一）', 'あんこう鍋（冬）', '干し芋（冬）', '納豆'],
                    desc: '「日本一」を複数持つ食材王国！特にメロンとれんこんは全国シェアトップクラスです。'
                },
                {
                    name: '栃木県', emoji: '🍓',
                    highlights: ['とちおとめいちご（春・生産量日本一）', 'スカイベリー（春）', '宇都宮餃子', '日光湯波', '那須和牛'],
                    desc: 'いちご王国として名高く、春のいちご狩りは関東随一の人気スポットです！'
                },
                {
                    name: '群馬県', emoji: '🌿',
                    highlights: ['こんにゃく（全国生産の約90%）', '下仁田ネギ（冬）', '嬬恋高原レタス（夏）', 'まいたけ（秋）', '上州和牛'],
                    desc: 'こんにゃくの聖地！下仁田ネギは加熱するとトロトロになる幻のブランドネギです。'
                },
                {
                    name: '埼玉県', emoji: '🍠',
                    highlights: ['狭山茶（春）', '川越さつまいも（秋）', '深谷ネギ（冬）', '草加せんべい', 'ゼリーフライ（行田）'],
                    desc: '小江戸・川越のさつまいもスイーツが話題！狭山茶は全国3大産地のひとつです。'
                },
                {
                    name: '千葉県', emoji: '🥜',
                    highlights: ['落花生（秋・生産量日本一）', '梨（夏・生産量日本一）', '房州びわ（初夏）', 'イワシ料理', 'さんが焼き'],
                    desc: '農業産出額は全国上位！落花生と梨はどちらも日本一の生産量を誇ります。'
                },
                {
                    name: '東京都', emoji: '🏙️',
                    highlights: ['江戸前あさり（春）', '東京うど（春）', '小松菜（冬）', 'くさや（伊豆諸島）', '深川めし'],
                    desc: '都市のイメージが強いですが、伝統野菜や江戸前の海鮮文化が今も息づいています！'
                },
                {
                    name: '神奈川県', emoji: '🐟',
                    highlights: ['三崎マグロ（冬）', '湘南しらす（春）', '三浦大根（冬）', '湘南ゴールド（春）', 'かまぼこ（小田原）'],
                    desc: '三崎港のマグロは全国屈指！春の生しらす解禁は毎年多くのグルメファンが訪れます。'
                },
            ];
            ...
            // キーワードパターンマッチを優先
            for (const { re, res: r } of chatPatterns) {
                if (re.test(text)) { res = r; break; }
            }

            // マッチしなかった場合 → ランダムに都県を選んで紹介
            if (!res) {
                const chosen = prefRandomData[Math.floor(Math.random() * prefRandomData.length)];
                const hlList = chosen.highlights.slice(0, 3).join('、');
                const intros = [
                    `今日は ${chosen.emoji} **${chosen.name}** を特集！`,
                    `🎲 ランダムピック → ${chosen.emoji} **${chosen.name}** はいかがでしょう？`,
                    `✨ おすすめは ${chosen.emoji} **${chosen.name}** です！`,
                ];
                const intro = intros[Math.floor(Math.random() * intros.length)];
                res = `${intro}\n\n${chosen.desc}\n\n🍽 代表的な食材：${hlList} など\n\n県名・季節・食材名で質問するとさらに詳しく答えられます！`;
            }

            botMsg.textContent = '';
            let i = 0;
            const typeInterval = setInterval(() => {
                botMsg.textContent += res[i]; i++;
                chatWindow.scrollTop = chatWindow.scrollHeight;
                if (i >= res.length) clearInterval(typeInterval);
            }, 25);
        }
        chatWindow.scrollTop = chatWindow.scrollHeight;
    };
